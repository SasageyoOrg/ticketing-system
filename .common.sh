...
if [ "${NO_LOCK_REQUIRED}" = "true" ];then
  if [ -f ${LOCK_FILE} ];then
    ...
    echo "Restart with    ->  ./runme.sh restart"
    echo "Resume with     ->  ./runme.sh resume"
    echo "Stop with       ->  ./runme.sh stop"
    echo "Remove with     ->  ./runme.sh remove " >&2
    exit 1
  fi
else
  composeFile=""
  if [ -f ${LOCK_FILE} ]; then
    ...
    echo "Blockchain is not running (${LOCK_FILE} not present)." >&2
    echo "Start it with -> ./runme.sh start" >&2
    exit 1
  fi
fi
